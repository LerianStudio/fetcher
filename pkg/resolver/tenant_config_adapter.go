...
// a given service, pickClient returns an explicit error.
...
// client. It returns an explicit error when neither is configured, so callers
// never dereference a nil client.
func (a *TenantManagerAdapter) pickClient(service string) (*tmclient.Client, error) {
	if c := a.clients[normalizeServiceToken(service)]; c != nil {
		return c, nil
	}

	if a.defaultClient != nil {
		return a.defaultClient, nil
	}
