...
func TestLoadServiceAPIKeysFromEnv(t *testing.T) {
	...
	t.Run("returns non-nil empty map when no service keys set", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.NotNil(t, got)
		// The prior subtest's t.Setenv values are already restored here, so no
		// MULTI_TENANT_SERVICE_API_KEY_* is set. Assert the contract is an empty
		// (len==0) usable map, which also proves t.Setenv isolation across subtests.
		assert.Empty(t, got)
		_, hasBare := got[""]
