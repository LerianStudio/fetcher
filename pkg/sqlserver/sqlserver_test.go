...
func TestValidateFieldsInSchemaSQLServer_CaseInsensitivity(t *testing.T) {
	tests := []struct {
		name           string
		...
		},
		{
			...
			expectedFields: []string{"user_id", "AccountName"},
			schema: TableSchema{
				TableName: "accounts",
				Columns: []ColumnInformation{
					{Name: "USER_ID"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			var count int32
			missing := ValidateFieldsInSchemaSQLServer(tt.expectedFields, tt.schema, &count)
...
func TestValidateFieldsInSchemaSQLServer_CounterIncrement(t *testing.T) {
	...
	// First call: 2 fields
	ValidateFieldsInSchemaSQLServer([]string{"id", "name"}, schema, &count)
	if count != 2 {
		t.Errorf("count after first call = %d, want 2", count)
	}

	// Second call: 3 more fields (counter should accumulate)
	ValidateFieldsInSchemaSQLServer([]string{"id", "name", "email"}, schema, &count)
	if count != 5 {
		t.Errorf("count after second call = %d, want 5", count)
	}
}

func TestDefaultSchema_SQLServer(t *testing.T) {
	// SQL Server default schema should be "dbo"
	if DefaultSchema == "" {
		t.Error("DefaultSchema should not be empty for SQL Server")
	}
}
