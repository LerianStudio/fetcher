...
//
// TEMPORARY COMPATIBILITY ADAPTER: lib-commons currently exposes webhook HMAC
// helpers, but those bind HTTP webhook wire fields (X-Webhook-Signature plus
// timestamp/body). Fetcher's AMQP envelope must bind tenant ID, job ID, exchange,
// and routing key so cross-tenant or cross-route replay fails. Until lib-commons
// grows a queue-envelope signing primitive, this file is the canonical
// Fetcher-specific envelope; keep signing changes here and covered by
// security_envelope_test.go. See docs/compatibility-waivers.md.
