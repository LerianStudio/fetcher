...
func TestTenantContext_MissingJWTFallback(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Nil(t, db,
				"GetMongoFromContext must return nil database when no tenant context, triggering static fallback")
...
// tmcore.GetMongoFromContext returns nil (e.g., tenant not found,
// no mongo in context), the repository layer can fall back to static connections.
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.Nil(t, mongoDB,
				"GetMongoFromContext returns nil without real DB in context")
...
func TestSingleTenantFallback_AllInfrastructure(t *testing.T) {
	...
	t.Run("mongodb_falls_back_to_static_without_tenant", func(t *testing.T) {
		...
		db := tmcore.GetMongoFromContext(ctx)
		assert.Nil(t, db,
			"GetMongoFromContext must return nil when no tenant in context to trigger static fallback")
	})
