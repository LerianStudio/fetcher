...
// Works with MetricsProvider interface for flexibility with different metrics implementations.
...
func NewChaosAssertions(t *testing.T, metrics MetricsProvider) *ChaosAssertions {
	t.Helper()
...
// SLAValidationResult holds the result of SLA validation.
...
func (a *ChaosAssertions) ValidateAgainstSLA(thresholds SLAThresholds) SLAValidationResult {
	...
	if p50 := snapshot.P50(); p50 > 0 {
		if p50 > thresholds.MaxP50Latency {
			result.Passed = false
			result.Failures = append(result.Failures,
	...
	if thresholds.MinThroughputRPS > 0 && snapshot.GetTotalRequests() > 0 {
...
func MeasureSteadyState(metrics MetricsProvider) SteadyStateResult {
	snapshot := metrics.Snapshot()

	return SteadyStateResult{
		SuccessRate:    snapshot.SuccessRate(),
		AverageLatency: snapshot.AverageLatency(),
		RequestCount:   snapshot.GetTotalRequests(),
	}
}
...
func DocumentResult(t *testing.T, metrics MetricsProvider, result string) {
	t.Helper()

	snapshot := metrics.Snapshot()

	t.Logf("RESULT: %s", result)
	t.Logf("  Total Requests: %d", snapshot.GetTotalRequests())
	t.Logf("  Success Rate: %.2f%%", snapshot.SuccessRate())
	t.Logf("  Failed Requests: %d", snapshot.GetFailedRequests())
	t.Logf("  Timeout Requests: %d", snapshot.GetTimeoutRequests())
	...
	t.Logf("  Test Duration: %v", snapshot.TestDuration())
}

// DocumentErrorBreakdown logs error classification details.
func DocumentErrorBreakdown(t *testing.T, metrics MetricsProvider) {
	t.Helper()
...
type ChaosTestCase struct {
	...
	Validation  func(t *testing.T, metrics MetricsProvider)
}
