...
func (s *ChaosTestSuite) TestPostgresTimeout_ExtractionJobFails() {
	...
	// Document results
	chaos.DocumentResult(t, s.metrics, "PostgreSQL timeout correctly rejected job, recovery successful")
}
...
func (s *ChaosTestSuite) TestPostgresLatency_SlowExtractionCompletes() {
	...
	notification, err := s.eventConsumer.WaitForJobEvent(
		...
		config.JobCompletionTimeoutSlow, // Extended timeout
	)
	...
	chaos.DocumentResult(t, s.metrics, "PostgreSQL extraction completed despite latency")
