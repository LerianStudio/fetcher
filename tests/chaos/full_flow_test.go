...
func (s *ChaosTestSuite) TestFullFlow_ChaosMidExtraction() {
	...
	chaosConfig := chaos.DefaultLatencyConfig(
		chaos.LatencyMs(chaos.ChaosLatencyValues.High),
	...
	defer func() { _ = s.chaosInfra.ChaosOps.RemoveChaos(chaos.ServicePostgres, chaosConfig.Name) }()

	// Let chaos affect for a bit
	time.Sleep(chaos.ChaosInjectionDuration)

	// Remove chaos (also handled by defer for safety)
	t.Log("Removing chaos...")
	err = s.chaosInfra.ChaosOps.RemoveChaos(chaos.ServicePostgres, chaosConfig.Name)
	require.NoError(t, err)
	...
	chaos.DocumentResult(t, s.metrics, "Full flow handled mid-extraction chaos")
}
...
func (s *ChaosTestSuite) TestFullFlow_MultiPointChaos() {
	...
	// RabbitMQ latency
	rabbitmqChaos := chaos.DefaultLatencyConfig(
		chaos.LatencyMs(chaos.ChaosLatencyValues.Medium),
		chaos.LatencyMs(chaos.ChaosLatencyValues.Jitter),
	)
	rabbitmqChaos.Name = "rabbitmq_multipoint_latency"
	_, err = s.chaosInfra.ChaosOps.AddChaos(chaos.ServiceRabbitMQ, rabbitmqChaos)
	require.NoError(t, err)
	defer func() { _ = s.chaosInfra.ChaosOps.RemoveChaos(chaos.ServiceRabbitMQ, rabbitmqChaos.Name) }()

	// PostgreSQL latency
	postgresChaos := chaos.DefaultLatencyConfig(
		chaos.LatencyMs(chaos.ChaosLatencyValues.Medium),
		chaos.LatencyMs(chaos.ChaosLatencyValues.Jitter),
	)
	postgresChaos.Name = "postgres_multipoint_latency"
	_, err = s.chaosInfra.ChaosOps.AddChaos(chaos.ServicePostgres, postgresChaos)
	require.NoError(t, err)
	defer func() { _ = s.chaosInfra.ChaosOps.RemoveChaos(chaos.ServicePostgres, postgresChaos.Name) }()
	...
	chaos.DocumentResult(t, s.metrics, "System operated under multi-point chaos")
}
...
func (s *ChaosTestSuite) TestFullFlow_RecoveryValidation() {
	...
	_, err = s.chaosInfra.ChaosOps.AddChaos(chaos.ServicePostgres, chaosConfig)
	require.NoError(t, err)

	// Run operations during chaos to stress the system
	t.Log("Stressing system during chaos...")
	chaosDeadline := time.Now().Add(chaos.ChaosInjectionDuration)
	chaosOpsSuccess := 0
	chaosOpsFailed := 0
	...
	chaos.DocumentResult(t, s.metrics, "System returned to steady state after chaos removal")
}
