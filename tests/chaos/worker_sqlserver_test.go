...
func (s *ChaosTestSuite) TestSQLServerTimeout_ExtractionJobFails() {
	...
	toxic, err := s.chaosInfra.ChaosOps.AddChaos(chaos.ServiceSQLServer, chaosConfig)
	require.NoError(t, err)
	require.NotNil(t, toxic)
	defer func() { _ = s.chaosInfra.ChaosOps.RemoveChaos(chaos.ServiceSQLServer, chaosConfig.Name) }()
	...
	err = s.chaosInfra.ChaosOps.RemoveChaos(chaos.ServiceSQLServer, chaosConfig.Name)
	require.NoError(t, err)
	...
	chaos.DocumentResult(t, s.metrics, "SQL Server timeout correctly rejected job, recovery successful")
}
...
func (s *ChaosTestSuite) TestSQLServerLatency_SlowExtractionCompletes() {
	...
	t.Logf("Job completed in %v (latency chaos: %v)", duration, chaos.ChaosLatencyValues.Medium)

	// Cleanup
	err = s.chaosInfra.ChaosOps.RemoveChaos(chaos.ServiceSQLServer, chaosConfig.Name)
	require.NoError(t, err)

	chaos.DocumentResult(t, s.metrics, "SQL Server extraction completed despite latency")
}
