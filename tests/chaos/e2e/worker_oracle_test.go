...
func (s *ChaosTestSuite) TestOracleTimeout_ExtractionJobFails() {
	...
	require.NotNil(t, proxy, "Oracle proxy should exist")

	chaosConfig := helpers.DefaultTimeoutConfig(helpers.TimeoutMs(setup.ChaosTimeoutValues.Long))
	...
	helpers.DocumentResult(t, s.metrics, "Oracle timeout correctly rejected job, recovery successful")
}
...
func (s *ChaosTestSuite) TestOracleLatency_SlowExtractionCompletes() {
	...
	t.Logf("Job completed in %v (latency chaos: %v)", duration, setup.ChaosLatencyValues.Low)

	// Cleanup
	err = helpers.RemoveChaos(proxy, chaosConfig.Name)
	require.NoError(t, err)

	helpers.DocumentResult(t, s.metrics, "Oracle extraction completed despite high latency")
}
