...
func (s *ChaosTestSuite) TestMongoDBTimeout_ConnectionCreationFails() {
	...
	// Use specific method: GetMongoMainProxy (NOT GetMongoDBProxy)
	proxy := s.chaosInfra.GetMongoMainProxy()
	require.NotNil(t, proxy, "MongoDB proxy should exist")

	// Timeout that exceeds MongoDB connection timeout (10s)
	chaosConfig := helpers.DefaultTimeoutConfig(15000) // 15s timeout
	...
	// Document results
	helpers.DocumentResult(t, s.metrics, "MongoDB timeout handled gracefully, recovery successful")
}
...
func (s *ChaosTestSuite) TestMongoDBLatency_SlowJobStatusQueries() {
	...
	helpers.DocumentResult(t, s.metrics, "Job status queries succeeded despite MongoDB latency")
}
