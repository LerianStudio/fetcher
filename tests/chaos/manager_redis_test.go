...
func (s *ChaosTestSuite) TestRedisUnavailable_RateLimitingFallback() {
	...
	err = s.chaosInfra.ChaosOps.DisableService(chaos.ServiceRedis)
	require.NoError(t, err, "Failed to disable Redis proxy")
	defer func() { _ = s.chaosInfra.ChaosOps.EnableService(chaos.ServiceRedis) }()
	...
	err = s.chaosInfra.ChaosOps.EnableService(chaos.ServiceRedis)
	require.NoError(t, err)
	...
	// Document results
	chaos.DocumentResult(t, s.metrics, "Rate limiting continued via in-memory fallback when Redis was unavailable")
}
