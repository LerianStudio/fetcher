...
func (s *ChaosTestSuite) SetupTest() {
	...
	s.metrics = chaos.NewExtendedMetrics()
	s.metrics.StartTest()
...
func (s *ChaosTestSuite) waitForManagerReady() error {
	ctx, cancel := context.WithTimeout(s.ctx, config.ManagerReadyTimeout)
	defer cancel()

	ticker := time.NewTicker(config.ManagerReadyPollInterval)
	defer ticker.Stop()
...
func (s *ChaosTestSuite) waitForJobProcessing(jobID string, timeout time.Duration) error {
	...
	for {
		if time.Now().After(deadline) {
			return fmt.Errorf("timeout waiting for job %s to start processing (last error: %v)", jobID, lastErr)
		}
