...
func (s *ChaosTestSuite) TestMySQLTimeout_ExtractionJobFails() {
	...
	toxic, err := s.chaosInfra.ChaosOps.AddChaos(chaos.ServiceMySQL, chaosConfig)
	require.NoError(t, err)
	require.NotNil(t, toxic)
	defer func() { _ = s.chaosInfra.ChaosOps.RemoveChaos(chaos.ServiceMySQL, chaosConfig.Name) }()
	...
	err = s.chaosInfra.ChaosOps.RemoveChaos(chaos.ServiceMySQL, chaosConfig.Name)
	require.NoError(t, err)
	...
	chaos.DocumentResult(t, s.metrics, "MySQL timeout correctly rejected job, recovery successful")
}
...
func (s *ChaosTestSuite) TestMySQLBandwidth_ThrottledExtractionCompletes() {
	...
	notification, err := s.eventConsumer.WaitForJobEvent(
		...
		config.JobCompletionTimeoutSlow, // Extended timeout for bandwidth throttling
	)
	...
	chaos.DocumentResult(t, s.metrics, "MySQL extraction completed under bandwidth throttling")
}
