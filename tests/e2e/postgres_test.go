...
func TestPostgresExtraction_TransactionsTable(t *testing.T) {
	...
	consumer := queuekit.NewConsumer[e2eshared.JobNotification](t, backend).
		...
		WithTimeout(jobCompletionTimeout).
		Build()
	...
	if err != nil {
		...
		if jobErr != nil {
			...
			t.Logf("Job status after timeout: status=%s, resultPath=%s", job.Status, job.ResultPath)
		}
	...
	assert.Equal(t, "completed", job.Status, "job status should be completed")
	assert.NotEmpty(t, job.ResultPath, "result path should be set")

	t.Logf("Job completed successfully: status=%s, resultPath=%s", job.Status, job.ResultPath)
}
