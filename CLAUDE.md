...
**Context propagation:** Use `observability.NewTrackingFromContext(ctx)` (lib-observability v4) for logger, tracer, requestID. Start OTel spans: `ctx, span := tracer.Start(ctx, "service.op"); defer span.End()`.
