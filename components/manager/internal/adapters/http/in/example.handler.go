...
import (
	...
	exampleModel "github.com/LerianStudio/fetcher/pkg/model"
...
func (ex *ExampleHandler) CreateExample(p any, c *fiber.Ctx) error {
	...
	err := libOtel.SetSpanAttributesFromStruct(&span, "payload", payload)
	if err != nil {
		libOtel.HandleSpanError(&span, "Failed to convert payload to JSON string", err)
	...
	if err != nil {
		libOtel.HandleSpanError(&span, "Failed to create example on services", err)
...
func (ex *ExampleHandler) GetExampleByID(c *fiber.Ctx) error {
	...
	if err != nil {
		libOtel.HandleSpanError(&span, "Failed to retrieve example on query", err)
...
//	@Success		200				{object}	libPostgres.Pagination{items=[]exampleModel.ExampleOutput,page=int,limit=int}
...
func (ex *ExampleHandler) UpdateExample(p any, c *fiber.Ctx) error {
	...
	if err != nil {
		libOtel.HandleSpanError(&span, "Failed to update example", err)
...
func (ex *ExampleHandler) DeleteExampleByID(c *fiber.Ctx) error {
	...
	if os.Getenv("ENV_NAME") == "production" {
		libOtel.HandleSpanError(&span, "Failed to remove example: "+constant.ErrActionNotPermitted.Error(), constant.ErrActionNotPermitted)
	...
	if err := ex.ExampleCommand.DeleteExampleByID(ctx, id); err != nil {
		libOtel.HandleSpanError(&span, "Failed to remove example on database", err)
